// imagem padrao do usuario em Base64
const DEFAULT_USER_PROFILE = `data:image/png;base64,`

function setLocalStorageValue(key, json) {
    localStorage.setItem(key, JSON.stringify(json))
}

function getLocalStorageValue(key) {
    return JSON.parse(localStorage.getItem(key))
}

function setSessionStorageValue(key, json) {
    sessionStorage.setItem(key, JSON.stringify(json))
}

function getSessionStorageValue(key) {
    return JSON.parse(sessionStorage.getItem(key))
}

var usuarios = []

const errorMessage = document.querySelector(".error-message")

// validacao de username com regex
const usernameValidation = (username) => { 
    var regex = /^(?=[a-zA-Z0-9._]{3,20}$)(?!.*[_.]{2})[^_.].*[^_.]$/g
    return regex.test(username)
}

//validacao de senha com regex
const passwordValidation = (password) => {
    var regex = /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{8,}$/g
    return regex.test(password)
}

function registrarUsuario() {
    // pega os inputs
    const nameInput = document.querySelector("#name")
    const usernameInput = document.querySelector("#username")
    const passwordInput = document.querySelector("#password")

    // verifica se existe algum valor nos inputs
    if (nameInput.value && usernameInput.value && passwordInput.value) {
        // pega a array de usuarios atuais, se nao existir define como vazia
        usuarios = getLocalStorageValue("users")
        if (!usuarios) usuarios = []

        // verifica se ja existe um usuario com mesmo username
        if (usuarios.findIndex(x => x.username == usernameInput.value) == -1) { // se nao existir
            if (!usernameValidation(usernameInput.value)) { // verifica se usuario é valido pelo regex
                errorMessage.innerHTML = "Esse usuário é inválido"
                return;
            }
            if (!passwordValidation(passwordInput.value)) { // verifica se senha é valida pelo regex
                errorMessage.innerHTML = "Essa senha é inválida"
                return;
            }
            // cria o objeto do usuario
            const user = { id: usuarios.length+1, image: DEFAULT_USER_PROFILE, name: nameInput.value, username: usernameInput.value, password: passwordInput.value }
            // coloca o objeto do usuario na array
            usuarios.push(user)
            // atualiza a array no localStorage
            setLocalStorageValue("users", usuarios)

            // limpa os campos de input
            nameInput.value = ''
            usernameInput.value = ''
            passwordInput.value = ''

            // define o usuario logado como o criado agora
            setSessionStorageValue("loggedUser", user)
            // redireciona para o index.html
            window.location = "/index.html"
        } else { // se ja existir
            // mensagem de erro pq usuario existe
            errorMessage.innerHTML = "Esse usuário já existe"
        }
    }
}

// eventos para click do mouse e enter do teclado
document.querySelector("#register-submit").addEventListener("click", () => registrarUsuario())
document.addEventListener("keypress", (e) => {
    if (e.key === 'Enter') {
        registrarUsuario()
    }
})
